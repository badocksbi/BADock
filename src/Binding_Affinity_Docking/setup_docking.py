...
def parse_options():
    ...
    '''
    parser = optparse.OptionParser('setup_docking.py -i PPI_NAME -j JOB_PATH -p PDB_PATH')
...
class SetupDocking(object):
    '''
    Execute rigit body docking (PatchDock) and Fiberdock (refinement and dG computation)
    to the Binding Affinity Benchmark 1
    '''
    ...
    def run(self):
        ...
        # DOCK BOUND RECEPTOR AND LIGAND WITH PATCHDOCK TO GET THE DECOYS #
        try:
            ...
            exit()
        sys.stdout.write('SUCCESS: Docking Computed!\n')
        sys.stdout.flush()
    ...
    def __create_patchdock_params(self):
        ...
        params += 'clusterParams 0.1 4 2.0 8.0\n'
    ...
    def __execute_fiberdock(self):
        ...
        try:
            subprocess.check_output(nma_cmd % (self.receptor_pdb, os.path.join(self.job_path, self.ppi+'_r_u')), stderr=subprocess.STDOUT, shell=True)
            subprocess.check_output(nma_cmd % (self.ligand_pdb, os.path.join(self.job_path, self.ppi+'_l_u')), stderr=subprocess.STDOUT, shell=True)
        except Exception as e:
...
if __name__ == "__main__":
    ...
    setup_docking = SetupDocking(options.ppi_name, options.job_path, options.pdb_path) 
    setup_docking.run()