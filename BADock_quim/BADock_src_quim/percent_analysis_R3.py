...
def main():
    ...
    """
    results_dir = "R3_results" # Introduce your results folder path
    try:
        os.stat("temp")
    except:
        os.mkdir("temp")
    ...
    print('\n############# TOTAL ANALYSIS #############')
    T_filtered, T_percents = calculate_percents(total_dGs, total_predictions)
    T_filtered_CCharPPI, T_percents_CCharPPI = calculate_percents(total_dGs_CCharPPI, total_predictions_CCharPPI, CCharPPI=True)

    print('\n############# RIGID ANALYSIS #############')
    R_filtered, R_percents = calculate_percents(R_dG, R_predictions)
    R_filtered_CCharPPI, R_percents_CCharPPI = calculate_percents(R_dG_CCharPPI, R_predictions_CCharPPI, CCharPPI=True)

    print('\n############# FLEXIBLE ANALYSIS #############')
    F_filtered, F_percents = calculate_percents(F_dG, F_predictions)
    F_filtered_CCharPPI, F_percents_CCharPPI = calculate_percents(F_dG_CCharPPI, F_predictions_CCharPPI, CCharPPI=True)

    # Writing table of results
    output_file = join(results_dir,"percents_individual_predictions_complete.tsv")
    outfile = open(output_file, 'w')
    print_table(T_percents, T_filtered, T_percents_CCharPPI, T_filtered_CCharPPI, R_percents, R_filtered, R_percents_CCharPPI, R_filtered_CCharPPI, F_percents, F_filtered, F_percents_CCharPPI, F_filtered_CCharPPI, outfile)
    outfile.close()

    # Perform Mann Whitney
    mw_results_T = calculate_mann_whitney(T_filtered)
    mw_results_R = calculate_mann_whitney(R_filtered)
    mw_results_F = calculate_mann_whitney(F_filtered)
    print mw_results_T

    # Writing table of Mann Whitney results
    mann_whitney_file = join(results_dir,"mann_whitney_table.tsv")
    mwfile = open(mann_whitney_file, 'w')
    print_mann_whitney(mw_results_T, mw_results_R, mw_results_F, mwfile)
    mwfile.close()

    return


def calculate_percents(dGs_experimental, predictions, CCharPPI=False):
    '''
...
def calculate_mann_whitney(filtered):
    ...
    for potential in filtered:
        ...
        for limit in ('1_4', '2_8', '4_2', 'more'):
            ...
            command = 'Rscript temp/temp.R > temp/temp.out'
            os.system(command)
