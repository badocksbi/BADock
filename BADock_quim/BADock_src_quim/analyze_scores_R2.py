...
class AnalyzePPI(object):
    ...
    def run(self):
        ...
        energies_print = []
        total_classes = {'N': 0,'I': 0, 'P': 0, 'E': 0, 'total': 0}
        for ppi in ppi_fo:
            ppi_type = ppi.strip('\n').split('\t')[1]
            ...
            try:
                ...
                decoy_classes = ppi_results[5]
                total_classes['N'] += decoy_classes['N']
                total_classes['I'] += decoy_classes['I']
                total_classes['P'] += decoy_classes['P']
                total_classes['E'] += decoy_classes['E']
                total_classes['total'] += decoy_classes['total']
    ...
    def __on_interaction(self, ppi):
        ...
        try:
            decoy_classes = self.__analyze_classes_decoys(location_decoys)
            print("Total number of decoys = %s" %(decoy_classes['total']))
            print("Number of decoys N = %s" %(decoy_classes['N']))
            print("Number of decoys I = %s" %(decoy_classes['I']))
            print("Number of decoys P = %s" %(decoy_classes['P']))
            print("Number of decoys E = %s" %(decoy_classes['E']))
        ...
        return ppi, macrostate_N, macrostate_I, macrostate_P, macrostate_E, decoy_classes
    ...
    def __analyze_classes_decoys(self, location_decoys):
        '''
        Get the number of each class of decoy and the total number of decoys
        '''
        decoy_classes = {}
        decoy_classes['N'] = len(location_decoys['N'])
        decoy_classes['I'] = len(location_decoys['I'])
        decoy_classes['P'] = len(location_decoys['P'])
        decoy_classes['E'] = len(location_decoys['E'])
        decoy_classes['total'] = len(location_decoys['N']) + len(location_decoys['I']) + len(location_decoys['P']) + len(location_decoys['E'])
    ...
    def __print(self, energies):
        ...
        '''
        ene_fo = open('scores_locations_E+OX_std_PROOF.txt' , 'w')
        for energie in energies:
            ...
            for x in xrange(len(energie[1])):
                ...
                if x < len(energie[1])-1:
                    ene_fo.write('\t')
            ene_fo.write('\t%d\t%d\t%d\t%d\t%d' % (energie[5]['N'], energie[5]['I'], energie[5]['P'], energie[5]['E'], energie[5]['total']))
            ene_fo.write('\n')
        ene_fo.close()
